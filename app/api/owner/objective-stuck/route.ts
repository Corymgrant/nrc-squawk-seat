...
// job 2610 — OBJECTIVE C DRIVER, Stuck panel. Same owner-only token-gated
// proxy pattern as every other /api/owner/* route in this app. Backend
// (cockpit/owner_dash.py:owner_objective_stuck) is a read-only passthrough
// of objective_stuck_panel.py's nightly artifact — this route adds no logic
// of its own.
...
export async function GET() {
  ...
  try {
    const r = await fetch(`${BASE}/objective-stuck`, {
